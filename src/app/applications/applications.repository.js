...
class ApplicationsRepositories {
  ...
  async getApplicationByUserId(user_id) {
    const query = {
      text: "SELECT a.id, a.job_id, a.user_id, j.title as job_title FROM applications as a JOIN jobs as j ON a.job_id = j.id WHERE a.user_id = $1",
      values: [user_id],
    };
  ...
  async getApplicationByJobsId(jobs_id) {
    const query = {
      text: "SELECT a.id, a.job_id, a.user_id, j.title as job_title FROM applications as a JOIN jobs as j ON a.job_id = j.id WHERE a.job_id = $1",
      values: [jobs_id],
    };
